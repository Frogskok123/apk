...
void progressBar(long cur_size, long total_size)
{
    ...
    printf("\r%.2f%% %.2f/%.2fMB", percent * 100, numTotal, numShow, cur_size / 1024.0 / 1024.0, total_size / 1024.0 / 1024.0);
    fflush(stdout);
...
void * download(void * socket_d)
{
    ...
    //创建文件描述符
    remove(dir);
    int fd = open(dir, O_CREAT | O_WRONLY, S_IRWXG | S_IRWXO | S_IRWXU);
...
int * down(char * urll)
{
    ...
    char file_name[256] = {0};
       ...
       // strcpy(url, app_update_url);
 
    parse_url(url, domain, &port);
