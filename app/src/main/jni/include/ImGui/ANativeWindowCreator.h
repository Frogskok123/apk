...
// Макрос для безопасного поиска функций
#define ResolveMethod(ClassName, MethodName, Handle, MethodSignature) \
    ...
    if (!ClassName##__##MethodName) { \
        __android_log_print(ANDROID_LOG_WARN, "ImGui", "[-] Method not found: %s", MethodSignature); \
    }
...
namespace android {
    namespace detail {
        namespace ui {
            ...
            struct Size { int32_t width = -1; int32_t height = -1; };
            struct DisplayState {
            ...
            struct DisplayInfo {
                ...
                float xdpi{0}; float ydpi{0}; float fps{0}; float density{0};
                uint8_t orientation{0}; bool secure{false};
                nsecs_t appVsyncOffset{0}; nsecs_t presentationDeadline{0};
                uint32_t viewportW{0}; uint32_t viewportH{0};
            };
        ...
        // --- FIX: StrongPointer с деструктором (Критично для ABI Android 12+) ---
        // Без деструктора компилятор считает это тривиальным типом и возвращает через r0/x0.
        // С деструктором - через x8 (скрытый указатель), как и в системе.
        ...
        struct Functionals {
            ...
            // Используем void* для аргументов, которые менялись (Metadata)
            StrongPointer<void> (*SurfaceComposerClient__CreateSurface)(
                void *thiz, 
                void *name,     
                uint32_t w, 
                uint32_t h, 
                ...
                uint32_t flags, 
                void *parentHandle, 
                void *layerMetadata, // void* позволяет передать и объект, и указатель
            ...
            {
                // Определение версии (улучшенное)
                char sdk_str[32] = {0};
                __system_property_get("ro.build.version.sdk", sdk_str);
                int sdk_ver = atoi(sdk_str);
                
                if (sdk_ver >= 35) systemVersion = 15;
                else if (sdk_ver >= 34) systemVersion = 14; 
                else if (sdk_ver >= 33) systemVersion = 13;
                else if (sdk_ver >= 31) systemVersion = 12;
                else {
                ...
                // --- ПОЛНАЯ ТАБЛИЦА ПАТЧЕЙ (Вернул все версии) ---
                static std::unordered_map<size_t, std::vector<std::pair<void**, const char*>>> patchesTable = {
                    {16, {
                        {(void**)&LayerMetadata__Constructor, "_ZN7android3gui13LayerMetadataC2Ev"},
                        {(void**)&SurfaceComposerClient__CreateSurface, "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_7String8EjjiiRKNS_2spINS_7IBinderEEENS_3gui13LayerMetadataEPj"},
                    }},
                    {15, {
                        {(void**)&LayerMetadata__Constructor, "_ZN7android3gui13LayerMetadataC2Ev"},
                        {(void**)&SurfaceComposerClient__CreateSurface, "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_7String8EjjiiRKNS_2spINS_7IBinderEEENS_3gui13LayerMetadataEPj"},
                    ...
                    {14, { 
                        {(void**)&LayerMetadata__Constructor, "_ZN7android3gui13LayerMetadataC2Ev"},
                        // HyperOS / Modern Android 14
                        {(void**)&SurfaceComposerClient__CreateSurface, "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_7String8EjjiiRKNS_2spINS_7IBinderEEENS_3gui13LayerMetadataEPj"},
                        // AOSP Android 14 Fallback
                        {(void**)&SurfaceComposerClient__CreateSurface, "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_7String8EjjijRKNS_2spINS_7IBinderEEENS_3gui13LayerMetadataEPj"},
                    }},
                    {12, { 
                        {(void**)&SurfaceComposerClient__Transaction__Apply, "_ZN7android21SurfaceComposerClient11Transaction5applyEb"},
                        // Исправленная сигнатура для 12/13
                        {(void**)&SurfaceComposerClient__CreateSurface, "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_7String8EjjijRKNS_2spINS_7IBinderEEENS_13LayerMetadataEPj"},
                    }},
                    {11, {
                        {(void**)&SurfaceComposerClient__CreateSurface, "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_7String8EjjijPNS_14SurfaceControlENS_13LayerMetadataEPj"},
                        {(void**)&SurfaceControl__GetSurface, "_ZNK7android14SurfaceControl10getSurfaceEv"},
                    }},
                    {10, {
                        {(void**)&SurfaceComposerClient__CreateSurface, "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_7String8EjjijPNS_14SurfaceControlENS_13LayerMetadataE"},
                        {(void**)&SurfaceControl__GetSurface, "_ZNK7android14SurfaceControl10getSurfaceEv"},
                    }},
                    {9, {
                        {(void**)&SurfaceComposerClient__CreateSurface_and9, "_ZN7android21SurfaceComposerClient13createSurfaceERKNS_7String8EjjijPNS_14SurfaceControlEii"},
                        {(void**)&SurfaceComposerClient__GetBuiltInDisplay, "_ZN7android21SurfaceComposerClient17getBuiltInDisplayEi"},
                        {(void**)&SurfaceControl__GetSurface, "_ZNK7android14SurfaceControl10getSurfaceEv"},
                    }},
                };
...
#endif
                // Фолбек, если стандартные пути не сработали
                if (!libgui) libgui = symbolMethod.Open("libgui.so", RTLD_LAZY);
                if (!libutils) libutils = symbolMethod.Open("libutils.so", RTLD_LAZY);
                ...
                // --- ЛОГИКА ПАТЧЕЙ (Оригинальная + Дополненная) ---
                std::vector<size_t> versionsToTry;
                versionsToTry.push_back(systemVersion);
                // Для HyperOS/14 полезно проверить и 12, если нативные символы не найдены
                if (systemVersion >= 14) versionsToTry.push_back(12);

                bool foundCreate = false;
                for (size_t ver : versionsToTry) {
                    if (patchesTable.count(ver)) {
                        for (const auto &[patchTo, signature] : patchesTable.at(ver)) {
                            // Если символ уже найден (например, для 14 версии), не перезаписываем его версией 12
                            if (*patchTo != nullptr && ver < systemVersion) continue;

                            void* addr = symbolMethod.Find(libgui, signature);
                            if (addr) {
                                *patchTo = addr;
                                if (patchTo == (void**)&SurfaceComposerClient__CreateSurface) foundCreate = true;
                            }
                        }
                    }
                }
        ...
        struct LayerMetadata {
            ...
            LayerMetadata() {
                ...
                if (9 < Functionals::GetInstance().systemVersion && Functionals::GetInstance().LayerMetadata__Constructor) {
        ...
        struct SurfaceComposerClientTransaction {
            ...
            int32_t Apply(bool synchronous, bool oneWay) {
                if (12 <= Functionals::GetInstance().systemVersion)
                    // FIX: Android 12+ принимает (void*, bool)
        ...
        struct SurfaceComposerClient {
            ...
            SurfaceControl CreateSurface(const char *name, int32_t width, int32_t height, bool skipScrenshot) {
                ...
                if (Functionals::GetInstance().systemVersion == 9) {
                    ...
                    auto createFn = funcs.SurfaceComposerClient__CreateSurface;

                    if (createFn) {
                        // --- ЛОГИКА ДЛЯ Android 14 / HyperOS ---
                        // Передаем указатели (&) для сложных объектов (LayerMetadata, String8, IBinder).
                        // Это эмулирует ABI передачи по значению на ARM64.
                        uint32_t outTransformHint = 0;
                        result = createFn(
                            data, 
                            &windowName, // Pointer to String8
                            width, height, 1, flags, 
                            &parentHandle, // Pointer to Parent
                            &layerMetadata.data, // Pointer to Metadata
                            &outTransformHint // Pointer to Hint (FIX SEGFAULT)
                        );
                    } else {
                        // Fallback для старых версий (10, 11) - вызываем "как есть" (но это редко срабатывает для 14+)
                         // Заглушка, если createFn пуст, но версия >= 10 (должен был найтись в патчах)
                         __android_log_print(ANDROID_LOG_ERROR, "ImGui", "[-] CreateSurface function pointer is null!");
                    }
                }
                
                if (12 <= Functionals::GetInstance().systemVersion && result.get()) {
            ...
            bool GetDisplayInfo(ui::DisplayState *displayInfo) {
                ...
                if (defaultDisplayToken.get() == nullptr) {
                    if (9 >= funcs.systemVersion) { 
                        defaultDisplayToken = funcs.SurfaceComposerClient__GetBuiltInDisplay(ui::DisplayType::DisplayIdMain);
                    } else {
                        if (14 > funcs.systemVersion) {
                            defaultDisplayToken = funcs.SurfaceComposerClient__GetInternalDisplayToken();
                        } else { 
                            // Android 14+ Logic
                            if (funcs.SurfaceComposerClient__GetPhysicalDisplayIds) {
                                auto displayIds = funcs.SurfaceComposerClient__GetPhysicalDisplayIds();
                                if (!displayIds.empty() && funcs.SurfaceComposerClient__GetPhysicalDisplayToken)
                                    defaultDisplayToken = funcs.SurfaceComposerClient__GetPhysicalDisplayToken(displayIds[0]);
                            }
                        }
                    }
                }

                if (nullptr == defaultDisplayToken.get()) return false;

                if (11 <= funcs.systemVersion) {
                    if (funcs.SurfaceComposerClient__GetDisplayState)
                        return 0 == funcs.SurfaceComposerClient__GetDisplayState(defaultDisplayToken, displayInfo);
                } else { 
                    ui::DisplayInfo realDisplayInfo{};
                    if (0 != funcs.SurfaceComposerClient__GetDisplayInfo(defaultDisplayToken, &realDisplayInfo))
                        return false;
    ...
    public:
        // Возвращена оригинальная структура DisplayInfo для совместимости с вашим кодом
        struct DisplayInfo {
            int32_t orientation;
            int32_t width;
            int32_t height;
